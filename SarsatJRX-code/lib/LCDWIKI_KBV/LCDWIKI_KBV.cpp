...
void LCDWIKI_KBV::Set_Addr_Window(int16_t x1, int16_t y1, int16_t x2, int16_t y2)
{
	...
	else if(lcd_driver == ID_NT35510)
	{
		writeCmdData16(XC,x1>>8);
		writeCmdData16(XC+1,x1);
        writeCmdData16(XC+2,x2>>8);
	    writeCmdData16(XC+3,x2);

		writeCmdData16(YC,y1>>8);
		writeCmdData16(YC+1,y1);
		writeCmdData16(YC+2,y2>>8);
		writeCmdData16(YC+3,y2);
	}
	else
	{
...
uint16_t LCDWIKI_KBV::Read_ID(void)
{
	...
	if((Read_Reg(0xC500,0) == 0x55) && (Read_Reg(0xC501,0) == 0x10))
	{
		return 0x5510;
	}
...
*/
		...
		case 0x7735:
			...
			break;
		case 0x5510:
			lcd_driver = ID_NT35510;
			XC=0x2A00,YC=0x2B00,CC=0x2C00,RC=0x2E00,SC1=0x33,SC2=0x37,MD=0x3600,VL=0,R24BIT=0;
		    static const uint16_t NT35510_regValues[] PROGMEM = 
			{			
				0xF000, 0x55,
				0xF001, 0xAA,
				0xF002, 0x52,
				...
				//## SDT:
				0xB600, 0x08,
				0xB500, 0x50,//0x6b ???? 480x854		0x50 ???? 480x800
				...
				0x3500, 0x00,
				0x3600, 0x00,
				0x3a00, 0x55,	////55=16?/////66=18?
			};
			init_table16(NT35510_regValues, sizeof(NT35510_regValues));
			Write_Cmd(0x1100);
			delay(100);
			Write_Cmd(0x2900);
			delay(50);
